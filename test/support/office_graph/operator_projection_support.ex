defmodule OfficeGraph.TestSupport.OperatorProjectionSupport do
  ...
  def submit_manual_intake(session, key) do
    {:ok, operation} =
  ...
  def apply_changes(session, proposed_changes) do
  ...
  def create_read_only_session!(bootstrap) do
    create_session_with_capabilities!(bootstrap, ["skeleton.read"])
  end

  def create_session_with_capabilities!(bootstrap, capability_keys) do
  ...
  def packet_default_value(command_affordance, field) do
  ...
  def packet_default_values(command_affordance, field) do
    command_affordance.input_defaults
    |> Enum.find(&(&1.field == field))
    |> case do
      nil -> []
  ...
  def start_run_for_packet_version(session, packet_version, key) do
    {:ok, run_operation} =
  ...
  def create_packet_with_sources_and_checks(session, key, source_ids, check_ids) do
    {:ok, operation} =
  ...
  def create_next_packet_version(session, packet_id, current_version, check, index) do
    packet =
  ...
  def complete_linked_run(session, run, verification_check, key, "verified") do
  ...
  def complete_linked_run(session, run, verification_check, key, "failed") do
