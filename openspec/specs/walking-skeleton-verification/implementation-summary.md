...
| Stable product mutations route through Ash while remaining direct database paths burn down as unapproved debt | `OfficeGraph.WorkGraph` Ash create/update helpers, `database-access-debt.json` | `mix credo --strict --only OfficeGraph.Credo.Check.ProjectBoundaries` |
| Raw SQL requires exact user approval and currently has no approved exception | `approved-database-exceptions.json` | `mix credo --strict --only OfficeGraph.Credo.Check.ProjectBoundaries` |
