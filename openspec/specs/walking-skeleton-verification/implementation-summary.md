...
| Stable product mutations route through Ash and current database-boundary occurrences are checked directly against the approved-exception inventory | `OfficeGraph.WorkGraph` Ash create/update helpers, `approved-database-exceptions.json` | `mix credo --strict --only OfficeGraph.Credo.Check.ProjectBoundaries` |
| Raw SQL requires exact user approval and is limited to the two approved PostgreSQL 18 UUIDv7 migration fragments | `approved-database-exceptions.json` | `mix credo --strict --only OfficeGraph.Credo.Check.ProjectBoundaries` |
