...
| Stable product mutations route through Ash while remaining direct database paths burn down as unapproved debt | `OfficeGraph.WorkGraph` Ash create/update helpers, `database-access-debt.json` | `mix office_graph.database_boundaries` |
| Raw SQL requires exact user approval and currently has no approved exception | `approved-database-exceptions.json` | `mix office_graph.database_boundaries` |
