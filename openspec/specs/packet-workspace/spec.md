...
packet projection or command path. The first visible packet SHALL be displayed
only when `packetId` is absent without adding that default to the URL. Every
present `packetId` value SHALL remain the requested selection until existing
authorized detail behavior resolves it, and
...
- **THEN** the route MUST display the first packet on that page without adding
  `packetId` to the URL
...
- **WHEN** `/packets?packetId=<opaque-relay-id>` names a packet on the current
  authorized page
- **THEN** the route MUST select that packet and render its
...
- **THEN** the route MUST update `packetId` in the URL and render detail from
  the selected packet's Relay-owned data

#### Scenario: Pagination has no explicit selection

- **WHEN** the route loads another page while `packetId` is absent
- **THEN** the route MUST display the first packet on the new page or an empty
