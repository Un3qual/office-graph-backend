...
### Requirement: Initiative Project Work Containers
Office Graph SHALL model initiatives/projects as bounded work containers for
...
- **THEN** the work MUST be representable as an initiative/project scoped to
...
execution lanes inside an initiative/project.
...
- **THEN** each lane MUST be representable as a workstream with organization,
  workspace, initiative/project, owner or source, lifecycle state, and related
...
- **THEN** the item MUST be able to reference the applicable workstream without
  losing its initiative/project and workspace scope
...
- **WHEN** a design system, campaign, social calendar, finance account,
  ...
  an initiative
- **THEN** it MUST be attachable to the initiative/project or workstream as a
...
into a larger initiative/project.
...
- **THEN** the system MUST allow it to be promoted or linked into an
  initiative/project with provenance back to the original graph item
...
- **WHEN** a workspace, initiative/project, or workstream is created
...
- **WHEN** a graph item belongs to a workspace, initiative/project, or
  workstream
