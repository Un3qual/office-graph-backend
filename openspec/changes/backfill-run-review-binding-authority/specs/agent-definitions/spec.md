...
- **WHEN** the forward reconciliation migration encounters an existing active
  binding whose agent principal has the former workspace-scoped system-role
  authority
- **THEN** the existing binding, principal, role, assignment, and lifecycle
  identities MUST be preserved while that exact role receives the canonical
  model-generation, proposal, and evidence-suggestion capabilities
- **AND** unrelated roles and broader scopes MUST NOT receive those grants
