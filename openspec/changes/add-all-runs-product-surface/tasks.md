...
- [ ] 3.1 Create the route-owned `assets/app/routes/runs/` package, read-only Relay documents, canonical `/runs` registration with no alias/compatibility route or query, and global stylesheet integration without Tailwind or a route-specific UI framework.
