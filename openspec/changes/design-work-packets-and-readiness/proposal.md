...
- Define handoff contracts from work packets to agent runtime, human assignees,
  proposed graph changes, future runs, future verification, and API/realtime UI
  projections.
...
- `execution-package-handoffs`: contracts for handing packets to humans,
  internal agents, future run records, proposed graph changes, evidence
...
- None. Existing graph, verification, agent runtime, governance,
  proposed-change, API/realtime, and persistence specs remain source
...
- Consumes requirements from work graph, verification, agent runtime, enterprise
  governance, proposed graph changes, code organization, persistence, and
