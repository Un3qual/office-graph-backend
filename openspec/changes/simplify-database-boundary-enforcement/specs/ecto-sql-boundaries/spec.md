...
- **WHEN** tracked source or compiled project code uses runtime source
  ...
  any Agent MFA executor, Elixir or OTP gen-server or gen-state-machine startup
  with an opaque or dynamic callback provider, exported
  `:erpc.execute_call`/`execute_cast`, a
