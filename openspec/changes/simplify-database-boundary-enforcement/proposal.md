...
- Reject unsupported indirection and executable-script syntax instead of
  extending the gate with shell, JavaScript, Python, alias-flow, callback, or
...
- Delivers the replacement as three stacked reviews: bounded source and BEAM
  dependency gates, migration and terminal conformance, then deletion of the
  obsolete symbolic analyzers and synthetic semantic tests.
- Retains the existing Credo repository gate and approved-exception inventory
  format. The old gate remains active until the final stack entry proves the
  replacements and removes it.
