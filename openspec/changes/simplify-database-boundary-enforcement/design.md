...
The source layer scans every tracked `.ex`, `.exs`, and SQL-like project source regardless of its directory or extension casing, including compound SQL template suffixes such as `.sql.eex`, runtime Elixir, tests, test support, Mix tasks, configuration, seeds, migrations, and scripts. It parses Elixir with `Code.string_to_quoted` for line metadata, but it does not evaluate helper bodies, callbacks, conditionals, quoted code, SQL strings, or macro output.

It reports exact occurrences for direct and imported uses of known low-level primitives: `OfficeGraph.Repo`, including `load/2`, `Ecto.Adapters.SQL`, `Ecto.Migrator`, private migration and repository execution namespaces such as `Ecto.Migration.Runner` and `Ecto.Repo.Supervisor`, `Postgrex` including its Notifications and SimpleConnection execution namespaces, `Ecto.Multi`, local or qualified `Ecto.Query.API.fragment`/`unsafe_fragment`, SQL-bearing Ecto query options, SQL-bearing AshPostgres custom-index predicates and expression fields, AshPostgres check-constraint expressions, resource-level `calculations_to_sql`, `identity_wheres_to_sql`, `base_filter_sql`, and `create_table_options` values, verbatim AshPostgres migration-default overrides, migration `execute`/`execute_file`/`fragment`, migration SQL options, direct migration inserts, and repository transaction/connection control. SQL-like tracked files are raw-SQL occurrences by definition and use an exact whole-file fingerprint that can match only an explicitly approved exception. Import tracking preserves both `only` and arity-specific `except` declarations, so excluded database-shaped names remain ordinary local calls while included imports remain audited. Dynamic `apply`, module-function-argument process/task/RPC/`proc_lib`/`:erlang.hibernate` dispatch including every Agent MFA executor and exported `:erpc.execute_call`/`execute_cast` form, and wildcard-imported dispatcher calls, statically visible Elixir and OTP supervisor child specs started through `start_child` or `Supervisor.start_link/2` including standard module and `{module, argument}` forms, direct `Supervisor.child_spec/2` callbacks on persistence modules, static persistence callback modules passed to `Supervisor.start_link/3`, `DynamicSupervisor.start_link/3`, or `:supervisor.start_link/2,3`, EEx and IEx compilation/evaluation/recompilation entrypoints, direct and indirect Mix task or project-file evaluation through `Mix.Tasks.Run`, `Mix.Tasks.Eval`, `Mix.Task`, or `Mix.Project.in_project/3,4`, low-level `:elixir` quoted/form evaluators and `:elixir_compiler` execution entrypoints, Erlang shell, `:compile`, and `:make.all/0,1`, `:make.all_or_nothing/0`, and `:make.files/1,2` source compilation/loading helpers, `Config.Reader` evaluation or nonliteral/external config reads, runtime macro expansion, Erlang expression evaluation including `:file.eval`/`script`/`path_eval`/`path_script`, `Code.eval_quoted_with_env`, `Kernel.ParallelCompiler` source-compilation entrypoints, dynamic receivers with database operations, `Module.concat`, external SQL files, repository variable receivers, direct repository calls inside migrations, nonliteral migration or AshPostgres SQL-option containers, Elixir and Erlang runtime code loading or code-path mutation, database-mutating CLI subprocesses, Mix shell execution including `Mix.shell().cmd`, shell/interpreter dispatch, static `cmd` strings in the project `:aliases` configuration are fingerprinted for exact approval, with visible database clients classified as raw SQL; inline evaluation, dynamic alias containers, `mix do` task composition, arbitrary migration helper calls including anonymous-function invocation, compile/verification callbacks in every tracked module, untrusted or dynamic `@derive` providers, and migration branching/control flow including short-circuit operators are rejected as unresolved escape paths. The overloaded `:erlang.spawn_request/4` form selects the local-with-options or remote-without-options MFA layout from the static third-argument shape and checks both candidates only when that discriminator is unresolved, so neither target can bypass source or compiled enforcement and a statically proven remote node is not treated as a callback. Mix alias inspection is lexically limited to the `aliases` entry and the conventional local `aliases/0` definition in tracked `mix.exs`; it rejects dynamic alias command containers and executable escape tasks without interpreting command or code bodies, while ordinary static task aliases and static tracked script paths remain inert references whose implementations are covered independently. AshPostgres custom-index, check-constraint, migration-default, and resource-level SQL-setting inspection is lexically limited to the corresponding DSL calls inside the `postgres` section of a tracked Ash resource, so unrelated application functions with the same names are not classified. Every static SQL-bearing setting is fingerprinted; a dynamic value or container fails closed rather than being interpreted. Static `Config.Reader` reads are limited to exact `Path.expand/2` forms anchored by the source file's `__DIR__` that resolve to the tracked canonical project config files; relative literals, aliased path modules, and paths resolving outside the project remain unresolved. Module availability checks against the unchanged deployment code path are not treated as repository-external source evaluation. Migration module bodies and transaction hooks are execution contexts subject to the same helper and control-flow restrictions as `change`, `up`, and `down`. Qualified `Ecto.Migration` calls receive the same enforcement as imported migration calls. Module-trust and local-definition discovery stop at quoted syntax and resume only through explicit unquote boundaries, so inert `defmodule` or `def` data cannot authorize a dependency provider or hide a live migration primitive. Opaque dependency macro capabilities introduced through `require`, `import`, or `use`, and protocol derivation through `@derive`, fail closed in every tracked source regardless of matching BEAM coverage unless the provider is an exact trusted framework module; project-authored providers are recognized from the repository-wide union of tracked executable module definitions, while an `OfficeGraph` namespace prefix without a tracked definition grants no trust. The database-owned `pg_dump` inspection call remains statically visible and read-only; mutable PostgreSQL clients and shell dispatch that could conceal them fail closed, and read-only command seams require the exact executable spelling rather than a matching basename. The existing test-only invocation of the tracked canonical `bin/verify` script remains an exact source-path and argument-shape seam, not a general interpreter allowance, and exact SHA-256 fingerprints bind that seam to `bin/verify` and its invoked migration-baseline script. The three private `Kernel.ParallelCompiler.compile_to_path/3` helpers required to generate BEAM audit fixtures are independent exact OpenSpec exceptions with empty terminal-object inventories and focused scanner, gate, and Credo behavior tests. Each approved call directly contains its byte-hash guard and complete generated-source SHA-256 allowlist, so changing the guard, fixture input, compiled path, or allowlist invalidates the exact exception even when terminal schema is unchanged; any new path, locator, call shape, or generated-source fingerprint remains unresolved.

Compile-time file loaders remain explicit forbidden primitives. The source gate
rejects IEx file-import helpers and `Config.import_config/1` unless the latter
resolves lexically to a tracked project configuration source or matches the
single canonical `config_env()` environment import in `config/config.exs` with
all candidate environment files tracked. AshPostgres `custom_statements`
payloads are scanned only inside that resource DSL section: literal `up` and
`down` values receive exact fingerprints, while dynamic payloads or section
containers fail closed. Source and BEAM function-capture classifiers also
route statically visible process and reflection targets through the same
forbidden-operation inventories and reject a dynamic operation on those
execution-capable namespaces. Project-module trust resolves nested shorthand
...
gen-server and gen-state-machine startup calls inspect their literal callback
provider in both source and BEAM abstract code: tracked project modules remain
ordinary callbacks, while dependency-owned or dynamic providers fail closed
without evaluating the callback. Compiled provider trust
...
Alias paths beginning with `__MODULE__` resolve that lexical prefix before
classifying repository receivers, including module-body calls that leave no
runtime BEAM call after compilation.
The common `load` name is classified as a repository operation only on an
explicit or database-shaped receiver; generic generated `apply(module, :load,
...)` calls remain ordinary dispatch rather than being mistaken for direct
repository access.
...
The standalone `mix static.analysis` alias first constructs production BEAMs, then force-compiles the current test project sources with warnings as errors before Credo runs. `.credo.exs` still requires the project-boundary source files when their modules are absent, but avoids redefining already compiled modules and invalidating BEAM provenance used by the compiled audit.
