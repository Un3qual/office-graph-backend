...
The standalone `mix static.analysis` alias force-compiles the current project sources with warnings as errors before Credo runs. `.credo.exs` still requires the project-boundary source files when their modules are absent, but avoids redefining already compiled modules and invalidating BEAM provenance used by the compiled audit.
