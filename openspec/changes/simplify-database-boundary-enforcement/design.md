...
Runtime dependency installation through `Mix.install/1,2`, native library loading through `:erlang.load_nif/2`, and Erlang `:core_transform` or `:parse_transform` compiler callbacks are unresolved execution capabilities. Static non-callback `@compile` options remain inert, while a dynamic `@compile` value fails closed because proving that it cannot select a transform would require evaluation.
