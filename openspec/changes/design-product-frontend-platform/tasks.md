...
- [ ] 1.1 Evaluate Relay against the current Absinthe/AshGraphql schema for object identity, pagination, fragments, generated TypeScript, mutation payloads, and test ergonomics.
- [ ] 1.2 Evaluate TanStack Query plus generated GraphQL operation types against the same operator workflow reads, command responses, realtime invalidation, and test requirements.
- [ ] 1.3 Record the selected GraphQL client model in the implementation notes and remove the rejected client path from the implementation scope.
...
- [ ] 2.3 Add `AppProviders.tsx` for React application wrappers only, including the selected GraphQL client provider and any required app/session context.
...
- [ ] 3.2 Replace old operator data hooks or mappers with the selected GraphQL client model.
...
- [ ] 5.2 Add the selected GraphQL compiler or codegen check to frontend verification.
