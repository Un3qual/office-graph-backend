...
- [ ] 1.3 Add an API compatibility ledger entry for each existing manual
  GraphQL/JSON surface that remains live, including owner, reason, replacement
  target, parity tests, and retirement condition.
...
- [ ] 1.7 Add app-shell verification proving `/operator` references built asset
  paths that the frontend build can produce.
- [ ] 1.8 Run baseline verification:
...
- [ ] 4.2 Add shared UI primitives for badge, button, panel, pane header, nav
  rail, text field, and empty/error state without embedding operator-workflow
  domain mapping logic.
- [ ] 4.3 Refactor `OperatorConsole.tsx` into route/container state, projection
...
- [ ] 4.4 Move operator-specific status-to-tone and action-label mapping into the
  operator-workflow feature boundary.
- [ ] 4.5 Introduce a projection-client interface for operator workflow data that
  returns frontend view models independent of JSON or GraphQL transport shape.
- [ ] 4.6 Keep the existing JSON adapter behind the projection-client interface
  and add adapter contract tests around the existing API fixtures.
- [ ] 4.7 Remove frontend-derived packet readiness command assembly from the
...
- [ ] 4.8 Add URL-selected inbox row behavior only if needed for the current
  operator route; defer broader React routing until a second real product route
  is accepted.
- [ ] 4.9 Add frontend tests for loading, empty, error, selection, readiness,
  run, verification, and app-shell asset behavior.
- [ ] 4.10 Run frontend verification through the project Nix shell and confirm it
  does not depend on global Node, npm, or TypeScript installs.
...
- [ ] 5.2 Update API and UI planning docs so Signal, Change Proposal, Work Item,
  Work Packet, Run, Check, Evidence, and Verification are the default MVP
  product spine.
...
- [ ] 5.4 Decide whether `proposed_graph_changes` receives an immediate code or
  storage rename, or whether API/UI translation plus a later storage migration
  is the safer path.
...
- [ ] 6.4 Run frontend verification and the app-shell asset test.
