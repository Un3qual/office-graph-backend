...
- [x] 6.3 Add failing Relay/operator tests, then implement the focused run-aware conversation, invocation, cancellation, approval, expansion, status, and error/conflict UI without agent administration or general chat.

## 7. First Automatic Agent

- [x] 7.1 Add end-to-end deterministic acceptance coverage for automatic run-review consuming authorized Office Graph context through the proposal path, plus focused output-router coverage for messages, findings, observations, and evidence candidates.
