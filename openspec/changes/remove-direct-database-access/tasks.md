...
- [x] 5.3 Replace execution-worker claim, completion, failure, and finalization SQL/transactions with lease-aware atomic actions and query locks
- [x] 5.4 Add separate-owner concurrency tests for invocation, binding, approval/context resolution, cancellation, leases, retry, and terminal transitions
...
- [x] 6.2 Replace outbound enqueue, webhook receipt, reconciliation, failure, and worker transaction wrappers with Ash actions and bulk/upsert behavior
- [x] 6.3 Add separate-owner concurrency tests for installation binding, webhook replay, reconciliation ordering, outbound idempotency, revocation, and failure atomicity
