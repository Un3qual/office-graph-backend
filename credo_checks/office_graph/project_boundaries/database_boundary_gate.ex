defmodule OfficeGraph.ProjectQuality.DatabaseBoundaryGate do
  ...
  @spec check_repository(Path.t(), keyword()) :: [map()]
  def check_repository(root \\ File.cwd!(), opts \\ []) do
    approved_path =
      Path.join(
        root,
    ...
    compiled =
      root
      |> DatabaseBoundaryScanner.scan_compiled(Keyword.get(opts, :compiled_audit, []))
  ...
  defp compiled_diagnostics(compiled, current) do
    source_keys = current |> Enum.map(&compiled_source_key/1) |> MapSet.new()
