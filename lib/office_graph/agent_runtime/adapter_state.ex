defmodule OfficeGraph.AgentRuntime.AdapterState do
  ...
  defmodule Entry do
    ...
    defstruct [:fingerprint, :result, :retained, :status]
  end
  ...
  def complete(namespace, key, fingerprint, result),
    do: complete(namespace, key, nil, fingerprint, result, nil)

  def complete(namespace, key, fingerprint, result, retained),
    do: complete(namespace, key, nil, fingerprint, result, retained)

  def complete(namespace, key, request_id, fingerprint, result, retained),
    do:
      GenServer.call(
        __MODULE__,
  ...
  def attach_cancel_target(namespace, request_id, target, cancel_ref)
    ...
    attach_cancel_target(namespace, request_id, target, cancel_ref, 5_000)
  end

  def attach_cancel_target(namespace, request_id, target, cancel_ref, timeout)
      when is_pid(target) and is_reference(cancel_ref) and is_integer(timeout) and timeout > 0 do
    GenServer.call(
      __MODULE__,
      ...
      %{fingerprint: ^fingerprint, owner: ^caller} = pending ->
        ...
        runtime =
          if retryable_result?(result) do
            ...
            |> put_record(
              pending.request_id,
              request_record(:retryable, key, fingerprint),
              retained
            )
      ...
      _pending ->
        case Map.get(runtime.entries, key) do
          %Entry{
            ...
            {:reply, {:replay, completed_result}, put_runtime(state, namespace, runtime)}
  ...
  def handle_call({:cancel, namespace, request_id}, _from, state) do
    ...
    case cancellable_request(runtime, request_id) do
      {:owner, key, pending} ->
        runtime = cancel_pending(runtime, key, pending)
        {:reply, :ok, put_runtime(state, namespace, runtime)}

      {:waiters, key, pending, waiters} ->
        Enum.each(waiters, &reply_waiter(&1, :cancelled))
        Enum.each(waiters, &Process.demonitor(&1.monitor, [:flush]))
        remaining_waiters = Enum.reject(pending.waiters, &(&1.request_id == request_id))
        runtime = put_in(runtime.pending[key].waiters, remaining_waiters)
        claim_ref = waiters |> List.first() |> Map.fetch!(:claim_ref)

        runtime =
          put_record(
            runtime,
            request_id,
            request_record(:cancelled, key, pending.fingerprint, claim_ref)
          )
  ...
  defp claim_terminal_or_new(
    ...
    case Map.get(runtime.entries, key) do
      %Entry{
        ...
        runtime = record_replay(runtime, request_id, key, fingerprint, claim_ref, retained)
  ...
  defp terminalize_timed_out_delivery(runtime, key, request_id, fingerprint, claim_ref) do
    case Map.get(runtime.requests, request_id) do
      ...
      when status in [:cancelled, :completed, :conflict, :replayed, :retryable] ->
        terminalize_request(runtime, request_id, :timed_out, key, fingerprint)
  ...
  defp record_replay(runtime, request_id, key, fingerprint, claim_ref, retained) do
    existing = Map.get(runtime.requests, request_id)

    record =
      if successful_record?(existing, key, fingerprint),
        do: bind(existing, :claim_ref, claim_ref),
        else: request_record(:replayed, key, fingerprint, claim_ref)

    put_record(runtime, request_id, record, retained)
  end
  ...
  defp put_record(runtime, request_id, record, retained) do
    retained_by_request =
      if is_nil(retained) do
        runtime.retained
      else
        Map.put(runtime.retained, request_id, retained)
      end
  ...
  defp cancellable_request(runtime, request_id) do
    Enum.find_value(runtime.pending, :none, fn {key, pending} ->
      cond do
        pending.request_id == request_id ->
          {:owner, key, pending}

        (waiters = Enum.filter(pending.waiters, &(&1.request_id == request_id))) != [] ->
          {:waiters, key, pending, waiters}
