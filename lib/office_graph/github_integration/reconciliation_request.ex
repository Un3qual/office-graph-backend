defmodule OfficeGraph.GitHubIntegration.ReconciliationRequest do
  ...
  defstruct [:installation_id, :object_type, :object_id, :pull_request_id, :delivery_id]

  @object_types ~w(pull_request review_comment check_run)
  @fields [:installation_id, :object_type, :object_id, :pull_request_id, :delivery_id]
  ...
  def new(attrs) when is_map(attrs) do
    ...
    with :ok <- required_uuid(request.installation_id, :installation_id),
         ...
         :ok <- valid_pull_request_scope(request),
  ...
  defp valid_pull_request_scope(%{object_type: "check_run", pull_request_id: nil}), do: :ok

  defp valid_pull_request_scope(%{object_type: "check_run", pull_request_id: pull_request_id}),
    do: required_string(pull_request_id, :pull_request_id)

  defp valid_pull_request_scope(%{pull_request_id: nil}), do: :ok

  defp valid_pull_request_scope(_request),
    do: {:error, {:invalid_reconciliation_request, :pull_request_id}}
