...
export function RunCommandForm({
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    command.submit({
      ...
      observationIdempotencyKey: `observation:${attempt.current.key}`,
    });
  };
  ...
  return (
    <form className="operator-command-form" onSubmit={submit} ref={formRef}>
      ...
      <select
        ...
        name="observationOutcomeKey"
        onChange={(event) => setSelectedOutcomeKey(event.target.value)}
        value={currentOutcomeKey}
      >
        {(currentOption?.outcomes ?? []).map((outcome) => (
      ...
      />
      <CommandFieldError
        controlName="observationRationale"
        scope="record-observation"
        state={command.state}
      />
      <Button
        isDisabled={command.state.status === "pending" || !rationale.trim() || options.length === 0}
        type="submit"
        variant="primary"
      >
...
function completeOption(option: object, fields: string[]) {
  ...
  if (
    !fields.every((field) => usableProjectionValue(objectValue(option, field))) ||
    !Array.isArray(outcomesValue) ||
    outcomesValue.length === 0
  )
    return false;

  const outcomes: unknown[] = outcomesValue;
  const keys = outcomes.map((outcome) =>
    typeof outcome === "object" && outcome !== null ? objectValue(outcome, "key") : null,
  );

  return (
    new Set(keys).size === keys.length &&
    keys.includes(objectValue(option, "defaultOutcomeKey")) &&
    outcomes.every(
      (outcome) =>
