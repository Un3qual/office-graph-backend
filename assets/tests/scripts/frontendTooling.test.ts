...
describe("frontend static verification", () => {
  ...
  it("runs pinned lint and formatting checks with generated outputs excluded", () => {
    ...
    expect(packageJson.scripts.lint).toBe(
      "biome lint app/relay app/routes src scripts tests *.ts *.json",
    );
    expect(packageJson.scripts["format:check"]).toBe(
      "biome format app/relay app/routes src scripts tests *.ts *.json",
    );
  ...
  it("keeps test files and test-only support in the dedicated test tree", () => {
    const misplacedTestFiles = ["app", "src", "scripts"]
      .flatMap((directory) => sourceFiles(join(root, directory)))
      .filter((file) => /\.(?:test|spec)\.[jt]sx?$|TestSupport\.[jt]sx?$/.test(file))
      .map((file) => relative(root, file));

    expect(misplacedTestFiles).toEqual([]);
    expect(existsSync(join(root, "tests/vitest.setup.ts"))).toBe(true);
    expect(existsSync(join(root, "vitest.setup.ts"))).toBe(false);
  });
